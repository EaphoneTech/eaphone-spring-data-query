...
public abstract class AbstractColumnTypeDecorator {
	...
	public static AbstractColumnTypeDecorator forType(Class<?> clazz, String attributeName) {
		...
		if (String.class.isAssignableFrom(fieldType) || Character.class.isAssignableFrom(fieldType)
			...
			return STRING;
		} else if (Byte.TYPE.equals(fieldType) || Short.TYPE.equals(fieldType) || Integer.TYPE.equals(fieldType)
				|| Long.TYPE.equals(fieldType)) {
			return INTEGER;
		} else if (Number.class.isAssignableFrom(fieldType) || Float.TYPE.equals(fieldType)
	...
	private static final Predicate or(CriteriaBuilder crit, @Nonnull Predicate pred1, @Nullable Predicate pred2) {
		if (pred2 == null) {
			return pred1;
		}
	...
	private static final AbstractColumnTypeDecorator STRING = new AbstractColumnTypeDecorator() {
		...
		public void fillOperations(List<BooleanExpression> ops, PathBuilder<Object> path, QueryFilter filter) {
			...
			if (StringUtils.hasLength(filter.get_notlike())) {
				ops.add(Expressions.stringOperation(Ops.LIKE, path).like(filter.get_notlike()).not());
			}
