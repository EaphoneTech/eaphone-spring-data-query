...
class ColumnSearchFilter extends GlobalFilter {
    ...
    public Predicate createPredicate(PathBuilder<?> pathBuilder, String attributeName) {
        ...
        if (addNullCase)
            predicate = predicate.or(path.isNull());
        return predicate;
    }

    @Override
    public javax.persistence.criteria.Predicate createPredicate(From<?, ?> from, CriteriaBuilder criteriaBuilder,
